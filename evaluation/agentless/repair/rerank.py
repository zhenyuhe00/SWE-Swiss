...
def majority_voting(args):
    ...
    with open(args.output_file, "w") as f:
        ...
        for instance_id in execution_results:
            ...
            patch_sim = []
            for str1 in patch_keys:
                tmp_sim = []
                for str2 in patch_keys:
                    ratio = Levenshtein.ratio(str1, str2)
                    tmp_sim.append(ratio)
                    tmp_sim = sorted(tmp_sim, reverse=True)[:args.num_samples//2]
                patch_sim.append(np.mean(tmp_sim))
            ...
            maj_selected_id = max(
                patch_ids,
                key=lambda i: (vote[patch_keys[i]] + patch_sim[i], patch_sim[i], -first_appear_idx[patch_keys[i]]),
            )
