...
def gen_test(instance_id, args, swe_bench_data, prev_o, write_lock=None, loc=None):
...
def generate_tests(args):
    ...
    if args.num_threads == 1:
        for instance_id in tqdm(instances, total=len(instances), colour="MAGENTA"):
            gen_test(instance_id, args, swe_bench_data, prev_o, loc=locs[instance_id])
    else:
        write_lock = Lock()
        with concurrent.futures.ThreadPoolExecutor(
            ...
            futures = {
                executor.submit(
                    gen_test, instance_id, args, swe_bench_data, prev_o, write_lock, loc=locs[instance_id]
                ): instance_id
